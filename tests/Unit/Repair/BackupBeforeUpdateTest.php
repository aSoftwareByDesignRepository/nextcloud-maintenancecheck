...
namespace OCA\MaintenanceCheck\Tests\Unit\Repair;
...
final class BackupBeforeUpdateTest extends TestCase
{
	...
	public function testPropagatesBackupFailure(): void
	{
		...
		$backup->method('createSnapshot')->willThrowException(
			new \OCA\MaintenanceCheck\Exception\UpgradeBackupException('disk full'),
		);

		$output = $this->createMock(IOutput::class);
		$output->expects(self::once())->method('warning');

		$this->expectException(\OCA\MaintenanceCheck\Exception\UpgradeBackupException::class);
