...
/**
 * MN2 wire format verification (SPEC §8.1). Uses the deterministic fixture
 * signing key via the MN_VENDOR_PUBLIC_KEY_B64 override.
 */
final class Mn2CodecTest extends TestCase
{
	...
	{
		$raw = file_get_contents(dirname(__DIR__, 2) . '/fixtures/license_mn2.json');
	...
	{
		$this->assertSame(self::$fixture['publicKeyB64'], Mn2TestSigning::publicKeyB64());
	}

	public function testFixtureWireKeyVerifies(): void
	{
		$wireKey = (string)self::$fixture['wireKey'];
		$this->assertSame('', Mn2Codec::classifyError($wireKey));
	...
	{
		$wireKey = Mn2TestSigning::signPayload($this->validPayload());
		$this->assertSame('', Mn2Codec::classifyError($wireKey));
	}
	...
	public function testWhitespaceInWireKeyIsTolerated(): void
	{
		...
		$this->assertSame('', Mn2Codec::classifyError($wrapped));
	}
	...
	public static function malformedKeys(): array
	{
		return [
			...
			'two parts' => ['MN2.onlypayload'],
			'four parts' => ['MN2.a.b.c'],
			'invalid base64 payload' => ['MN2.$$$$.AAAA'],
			'invalid base64 signature' => ['MN2.eyJ2IjoyfQ.$$$$'],
			'short signature' => ['MN2.eyJ2IjoyfQ.QUJD'],
		];
	}
	...
	{
		$this->assertSame(Mn2Codec::ERROR_INVALID_FORMAT, Mn2Codec::classifyError($wireKey));
		$this->assertNull(Mn2Codec::parseAndVerify($wireKey));
	}
	...
	public function testTamperedPayloadFailsSignature(): void
	{
		...
		$tamperedB64 = VendorPublicKey::base64urlEncode(Mn2Codec::canonicalJson($tampered));

		$this->assertSame(
			Mn2Codec::ERROR_INVALID_SIGNATURE,
			Mn2Codec::classifyError($fmt . '.' . $tamperedB64 . '.' . $sigB64),
		);
	}
	...
	public function testTamperedSignatureFails(): void
	{
		...
		$this->assertSame(
			Mn2Codec::ERROR_INVALID_SIGNATURE,
			Mn2Codec::classifyError($fmt . '.' . $payloadB64 . '.' . $flipped),
		);
	}
	...
	public function testKeySignedByDifferentKeyFails(): void
	{
		...
		$payloadBytes = Mn2Codec::canonicalJson($this->validPayload());
		$signature = sodium_crypto_sign_detached($payloadBytes, sodium_crypto_sign_secretkey($keypair));
		$wireKey = 'MN2.' . VendorPublicKey::base64urlEncode($payloadBytes)
			. '.' . VendorPublicKey::base64urlEncode($signature);

		$this->assertSame(Mn2Codec::ERROR_INVALID_SIGNATURE, Mn2Codec::classifyError($wireKey));
	}
	...
	public function testSignedButInvalidPayloadIsRejected(array $payload): void
	{
		...
		$wireKey = Mn2TestSigning::signRawBytes($json);
		$this->assertSame(Mn2Codec::ERROR_INVALID_PAYLOAD, Mn2Codec::classifyError($wireKey));
	}

	public function testValidPayloadBoundaries(): void
	{
		$this->assertTrue(Mn2Codec::validatePayloadFields(array_merge($this->validPayload(), ['mobileSeats' => 1])));
		$this->assertTrue(Mn2Codec::validatePayloadFields(array_merge($this->validPayload(), ['mobileSeats' => 10000])));
		$this->assertTrue(Mn2Codec::validatePayloadFields(array_merge($this->validPayload(), ['customerId' => 'abc'])));
		$this->assertTrue(Mn2Codec::validatePayloadFields(
	...
	{
		$wireKey = Mn2TestSigning::signRawBytes('this is not json');
		$this->assertSame(Mn2Codec::ERROR_INVALID_PAYLOAD, Mn2Codec::classifyError($wireKey));
	}

	public function testJsonScalarPayloadIsRejected(): void
	{
		$wireKey = Mn2TestSigning::signRawBytes('42');
		$this->assertSame(Mn2Codec::ERROR_INVALID_PAYLOAD, Mn2Codec::classifyError($wireKey));
	}
	...
	{
		$this->assertTrue(Mn2Codec::isValidOn('2027-12-31', '2027-12-30'));
		$this->assertTrue(Mn2Codec::isValidOn('2027-12-31', '2027-12-31'));
		$this->assertFalse(Mn2Codec::isValidOn('2027-12-31', '2028-01-01'));
	}
	...
	public function testExpiredKeyStillPassesStructuralVerification(): void
	{
		...
		$this->assertSame('', Mn2Codec::classifyError(Mn2TestSigning::signPayload($expired)));
	}

	public function testNormalizeWireKeyStripsAllWhitespace(): void
	{
		$this->assertSame('MN2.a.b', Mn2Codec::normalizeWireKey(" MN2\n.a\t. b \r\n"));
	}
	...
	public function testVendorKeyEnvOverrideIsHonouredUnderPhpunit(): void
	{
		...
		$this->assertSame(Mn2TestSigning::publicKeyB64(), VendorPublicKey::publicKeyB64());
		putenv('MN_VENDOR_PUBLIC_KEY_B64');
		try {
	...
	public function testProductionIgnoresEnvWhenOverrideFlagOff(): void
	{
		...
		// only MN_ALLOW_VENDOR_KEY_OVERRIDE=1 (or PHPUnit) may honour it.
		...
		putenv('MN_VENDOR_PUBLIC_KEY_B64=forged-not-a-real-key');
		try {
