#!/usr/bin/env node
/**
 * Behavioural tests for MaintenanceCheck app-feedback.js (mailto + toast hook).
 *
...
function boot(extra) {
	...
	const config = {
		appId: 'maintenancecheck',
		appDisplayName: 'MaintenanceCheck',
		appVersion: '1.0.0',
		feedbackEmail: 'dev@software-by-design.de',
		githubIssuesUrl: 'https://github.com/aSoftwareByDesignRepository/nextcloud-maintenancecheck/issues',
		cssPrefix: 'mn',
		...(extra || {}),
	};
	const cfgEl = {
	...
	nodes.set('mn-app-feedback-config', cfgEl);
	...
	nodes.set('mn-feedback-problem', problem);
	nodes.set('mn-feedback-idea', idea);

	const trigger = {
		className: 'mn-nav-footer__trigger',
		getAttribute(k) {
			return this[k] || null;
		},
		setAttribute(k, v) {
	...
	trigger['aria-controls'] = 'mn-feedback-menu';
	const menu = {
		id: 'mn-feedback-menu',
		hidden: true,
		contains() {
	...
	nodes.set('mn-feedback-menu', menu);
	doc.querySelector = (sel) => {
		if (sel === '.mn-nav-footer__trigger') {
			return trigger;
		}
		return null;
	};
	doc.getElementById = (id) => {
		if (id === 'mn-feedback-menu') {
			return menu;
		}
		return nodes.get(id) || null;
	};
	...
	const sandbox = {
		...
		location: {
			...
			pathname: '/apps/maintenancecheck/today',
			search: '?token=secret&keep=1',
			href: 'https://cloud.example/apps/maintenancecheck/today?token=secret&keep=1',
		},
...
function main() {
	...
	const clean = sandbox.SbdAppFeedback.sanitizePageUrl(
		'https://cloud.example/apps/maintenancecheck/x?token=abc&keep=1&password=no'
	);
	...
	const upgradeAck = sandbox.SbdAppFeedback.sanitizePageUrl(
		'/apps/maintenancecheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis'
	);
	assert(upgradeAck === '/apps/maintenancecheck/', 'strips Nextcloud web-upgrade ack query key');
	...
	process.stdout.write('app-feedback.test.js OK (maintenancecheck)\n');
}
