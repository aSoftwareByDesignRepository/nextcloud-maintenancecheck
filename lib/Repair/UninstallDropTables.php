 ...
 *
 * Drops every table the maintenancecheck app has ever created, migration rows, and app config.
 *
 ...
 *     php scripts/check-nextcloud-db-standards.php sync-uninstall --app=maintenancecheck
 *
...
final class UninstallDropTables implements IRepairStep
{
	...
	{
		return 'Drop maintenancecheck tables and install metadata on uninstall';
	}
	...
	public function run(IOutput $output): void
	{
		...
		$output->info(
			'maintenancecheck: preserving data on disable. '
	...
	private function dropAllTablesAndMetadata(IOutput $output): void
	{
		...
		$output->info(sprintf(
			'maintenancecheck: dropped %d of %d table(s); removed %d migration row(s), app config, and upgrade-backup snapshots.',
			$dropped,
			count(self::TABLES),
			$migrationsRemoved,
		));
	...
	private function purgeUpgradeBackupSnapshots(IOutput $output): void
	{
		...
		$node->delete();
		$output->info('maintenancecheck: removed upgrade-backup snapshots from app data.');
	}
}
