...
/**
 * Pre-migration repair step: snapshot MaintenanceCheck data before schema migrations run.
 *
 * Registered under {@see info.xml} `<repair-steps><pre-migration>` so every
 * `occ app:update maintenancecheck` and app reinstall over an existing version creates
...
final class BackupBeforeUpdate implements IRepairStep
{
	...
	{
		return 'Back up MaintenanceCheck data before update migrations';
	}

	public function run(IOutput $output): void
	{
		if (!$this->backupService->hasDataToBackup()) {
			$output->info('MaintenanceCheck: no existing tables to back up (fresh install); skipping pre-update snapshot.');
			return;
		}
		...
		try {
			...
			$output->warning('MaintenanceCheck: pre-update backup failed: ' . $e->getMessage());
			throw $e;
		}

		$tableCount = count($result['manifest']['tables'] ?? []);
		$output->info(sprintf(
			'MaintenanceCheck: pre-update backup created (%s, %d table(s)). '
			. 'Restore with `occ maintenancecheck:upgrade-backup restore --latest --force` if needed.',
			$result['id'],
			$tableCount,
		));
	}
