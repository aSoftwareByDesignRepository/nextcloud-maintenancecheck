...
/**
 * List, create, or restore MaintenanceCheck upgrade backups (CLI operator interface).
 *
...
{
	private const APP_LABEL = 'MaintenanceCheck';
	...
	{
		$this
			->setName('maintenancecheck:upgrade-backup')
			->setDescription('Manage MaintenanceCheck snapshots taken before app updates.')
			->setHelp(
				<<<'HELP'
...
<info>restore</info> — destructive; requires <comment>--force</comment> and snapshot id:
  <comment>occ maintenancecheck:upgrade-backup restore --latest --force</comment>
  <comment>occ maintenancecheck:upgrade-backup restore --id=YYYYMMDDTHHMMSSZ-hex --force</comment>
...
HELP
			...
			->addOption('force', null, InputOption::VALUE_NONE, 'Required for restore — overwrites current MaintenanceCheck data')
	...
	private function runList(SymfonyStyle $io): int
	{
		...
		if ($snapshots === []) {
			...
			$io->note('Create one manually: occ maintenancecheck:upgrade-backup create --reason="before change"');
	...
	private function runRestore(InputInterface $input, SymfonyStyle $io): int
	{
		...
		if (!$input->getOption('force')) {
			$io->error([
				'Restore overwrites all current MaintenanceCheck database rows, app config, and configured app-data folders.',
		...
		if ($snapshotId === '') {
			$io->error([
				...
				'  occ maintenancecheck:upgrade-backup restore --latest --force',
				'  occ maintenancecheck:upgrade-backup restore --id=<snapshot-id> --force',
				'Run `occ maintenancecheck:upgrade-backup list` to see snapshot ids.',
			]);
			return Command::FAILURE;
		}
